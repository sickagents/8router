...
Despliega 8Router en VPS o Docker para acceso remoto y uso en producción.
...
| `DATA_DIR` | `~/.8router` | Ruta de almacenamiento de la base de datos |
...
```bash
sudo mkdir -p /var/lib/8router
sudo chown $USER:$USER /var/lib/8router
```
...
Crea `/etc/nginx/sites-available/8router`:
...
sudo ln -s /etc/nginx/sites-available/8router /etc/nginx/sites-enabled/
...
# Si NO usas proxy reverso, permite los puertos de 8Router
...
tar -czf 8router-backup-$(date +%Y%m%d).tar.gz /var/lib/8router

# Respaldo automatizado diario (agregar a crontab)
0 2 * * * tar -czf /backups/8router-$(date +\%Y\%m\%d).tar.gz /var/lib/8router
```
...
```bash
# Verificar si 8Router está corriendo
pm2 status
...
sudo chown -R $USER:$USER /var/lib/8router
chmod 755 /var/lib/8router
```
