...
const has8RouterConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "8Router");
};

const get8RouterEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "8Router") || null;
};
...
export async function POST(request) {
  try {
    ...
    const idx = config.findIndex((e) => e.name === "8Router");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "8Router");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));
