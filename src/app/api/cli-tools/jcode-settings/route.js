...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-8router.env");
};
...
const has8RouterConfig = (config) => {
  ...
  if (providers["8router"]) return true;
...
export async function GET() {
  ...
  const has8Router = has8RouterConfig(config);
...
export async function POST(request) {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "jcode configured successfully. Use: jcode --provider-profile 8router",
      configPath: getConfigPath(),
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    delete config.providers["8router"];
    ...
    return NextResponse.json({
      success: true,
      message: "8router configuration removed from jcode",
    });
  } catch (error) {
