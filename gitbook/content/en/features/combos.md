...
Combos are **custom fallback chains** that you create in the dashboard. Instead of using a single model, you define a sequence of models that 8Router tries in order.
...
8Router automatically tries each model in sequence until one succeeds.
...
When limit reached, 8Router skips paid models and uses free tier only.
