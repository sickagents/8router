---
name: 8router-web-search
description: Web search via 8Router /v1/search using Tavily / Exa / Brave / Serper / SearXNG / Google PSE / Linkup / SearchAPI / You.com / Perplexity. Use when the user wants to search the web, look up information, find articles, or query a search engine.
---

# 8Router — Web Search

Requires `NINEROUTER_URL` (and `NINEROUTER_KEY` if auth enabled). See https://raw.githubusercontent.com/decolua/8router/refs/heads/master/skills/8router/SKILL.md for setup.
...
curl -X POST $NINEROUTER_URL/v1/search \
  ...
  -d '{"model":"tavily","query":"8Router open source","max_results":5}'
```
