...
function cleanupLegacySystray({ silent = false } = {}) {
  // 1) Runtime dir: ~/.8router/runtime/node_modules/systray (or %APPDATA% on Win)
  // 2) npm global nested: <npm_prefix>/node_modules/8router/node_modules/systray
...
function chmodSystrayBin({ silent = false } = {}) {
  ...
  try {
    ...
    if (!silent) console.warn(`[8router][runtime] chmod tray bin failed: ${e.message}`);
  }
}
