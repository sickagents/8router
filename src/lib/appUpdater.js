...
// Collect PIDs of all 8router-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from 8router install dir or wrapper cli.js
        const isAppProcess = lower.includes("8router") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("8router") ||
...
// Resolve npx/8router binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx 8router` — works regardless of global bin path changes after npm i -g
