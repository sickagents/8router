...
async function buildCodexHeader() {
  ...
  const { installed, has8Router, config } = result.data;
...
async function buildDroidHeader() {
  ...
  const { installed, has8Router, settings } = result.data;
  ...
  // Extract 8Router custom model config
  const custom = settings?.customModels?.find(m => m.id === "custom:8Router-0");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenClawHeader() {
  ...
  // Extract 8Router provider config
  const provider = settings?.models?.providers?.["8router"];
  const primary = settings?.agents?.defaults?.model?.primary || "";
  const model = primary.startsWith("8router/") ? primary.replace("8router/", "") : (provider?.models?.[0]?.id || "");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenCodeHeader() {
  ...
  const { installed, has8Router, opencode } = result.data;
...
async function showCliToolsMenu(port, breadcrumb = []) {
  ...
  await showMenuWithBack({
    title: "🔧 CLI Tools",
    breadcrumb,
    headerContent: `Configure CLI tools to use 8Router\nEndpoint: ${endpoint}`,
    items: [
      {
        label: "Claude Code",
