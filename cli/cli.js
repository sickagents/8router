...
// Self-heal SQLite runtime deps (sql.js + better-sqlite3) into ~/.8router/runtime
...
// Identifiers for killAllAppProcesses - only kill 8router specifically
const PROCESS_IDENTIFIERS = [
  '8router'  // Only package name - avoid killing other apps
];
    ...
    try {
      ...
      if (platform === "win32") {
        ...
        try {
          ...
          const lines = output.split("\n").slice(1).filter(l => l.trim());
          lines.forEach(line => {
            // Whitelist: real node process running 8router/cli.js, or next-server.
            // Avoids killing editors/grep/strace/cursor that just have "8router" in cmdline.
            const cmd = line.toLowerCase();
            const isAppProcess =
              (cmd.includes("node") && cmd.includes("8router") && (cmd.includes("cli.js") || cmd.includes("\\8router") || cmd.includes("/8router")))
        ...
        try {
          ...
          lines.forEach(line => {
            // Whitelist: real node process running 8router/cli.js, or next-server.
            // Avoids killing grep/strace/editors/cursor that incidentally match "8router".
            const cmd = line.toLowerCase();
            const isAppProcess =
              (cmd.includes("node") && cmd.includes("8router") && (cmd.includes("cli.js") || cmd.includes("/8router")))
              || cmd.includes("next-server");
            if (isAppProcess) {
              const parts = line.trim().split(/\s+/);
              const pid = parts[1];
      ...
      while (true) {
        ...
        if (choice === "update") {
          ...
          if (process.platform === "darwin") {
            ...
            console.log(`🔔 8Router is running in tray (PID: ${process.pid})`);
            console.log(`   Server: http://${displayHost}:${port}`);
          ...
          console.log(`🔔 8Router is now running in background (PID: ${bgProcess.pid})`);
          console.log(`   Server: http://${displayHost}:${port}`);
          console.log(`\n💡 You can close this terminal. Right-click tray icon to quit.\n`);
