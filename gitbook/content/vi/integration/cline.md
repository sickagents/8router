...
Tích hợp 8Router với extension Cline VSCode để định tuyến request AI qua hệ thống routing thông minh của 8Router.

## Yêu cầu

- Visual Studio Code đã cài đặt
- Extension Cline đã cài đặt từ VSCode marketplace
- 8Router đang chạy cục bộ hoặc cloud endpoint đã cấu hình
...
Đặt base URL tới endpoint 8Router:
...
3. Nhấn Enter để gửi
4. Cline sẽ dùng 8Router để xử lý request

### Tạo Code

1. Yêu cầu Cline tạo code: "Create a React component for a login form"
2. Cline sẽ tạo code qua 8Router
...
3. Nhận giải thích AI qua 8Router

### Thao tác File

1. Yêu cầu Cline tạo, sửa hoặc xóa files
2. Cline sẽ dùng 8Router để hiểu context và thực hiện thay đổi
...
1. Xác minh 8Router đang chạy: `curl http://localhost:20127/health`
...
2. Xác minh 8Router instance đang chạy và healthy
3. Thử reload cửa sổ VSCode (Cmd/Ctrl + Shift + P → "Reload Window")
4. Kiểm tra logs 8Router để xem lỗi

## Cấu hình Nâng cao

### Dùng Cloud Endpoint

Để dùng 8Router cloud endpoint thay vì localhost:

1. Trong Cline settings, đặt Base URL: `https://8router.com`
2. Đảm bảo bạn đã cấu hình API key trong 8Router cloud dashboard
...
## Tích hợp với Tính năng 8Router

### Định tuyến Model

8Router tự động định tuyến request đến provider tốt nhất hiện có dựa trên:
...
Nếu một provider thất bại, 8Router tự động fallback sang provider khác đã cấu hình trong dashboard.

### Theo dõi Usage

Giám sát usage Cline qua 8Router dashboard:
- Tổng request
- Token usage
- Chi phí mỗi model
- Phân bổ provider
