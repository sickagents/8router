...
// Check if settings has 8Router customModels
const has8RouterConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id?.startsWith("custom:8Router"));
};
...
// POST - Update 8Router customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing 8Router configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:8Router"));
...
// DELETE - Remove 8Router customModels only (keep other settings)
